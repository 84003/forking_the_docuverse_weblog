...
### Animating Multiple Instances of a Document

If a document has several content items linked to the open document, there will be multiple instances (previews) of that document on the sides. In the first image below, the multiple instances occur before a document is open, and in the second image it occurs after the middle document is open.
