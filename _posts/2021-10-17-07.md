...
[^2]: Hovering on a link to show (documents represented as) node in Vertical Mode is not particularly useful, since the size is only informative when seen in relation to other nodes. In the next round node representation is to be further developed.
